...
**Post-merge correction (2026-09-12):** while reviewing a sibling service (`product-catalog-service`) that copied this service's register-race pattern, the Acceptance Auditor layer surfaced that `User.id` being a client-assigned UUID (no `@GeneratedValue`) makes Spring Data JPA's `save()` defer its actual INSERT to transaction-commit time — *after* `AuthService.register`'s try/catch had already returned — so a concurrent duplicate-email race's constraint violation escaped uncaught and surfaced as an unhandled 500, not the intended 409. This was **not caught by my earlier "patch" verification** for that same race (2026-09-09): I tested sequential duplicate registration only, which hits the `existsByEmail` pre-check and never exercises the actual race. Built a real 8-thread concurrency test against a live Postgres (Testcontainers) and confirmed the bug empirically (0/7 losing requests caught) before fixing it.

Fix: `User` now implements `Persistable<UUID>` (documented reasoning in its javadoc) **and** `AuthService.register` uses `userRepository.saveAndFlush(...)` instead of `save(...)` — `Persistable` alone was insufficient, since Hibernate still defers the INSERT for a client-assigned-id entity with no forcing dependency; the explicit flush is what makes the constraint violation happen synchronously, inside the try/catch. Added `ConcurrentRegistrationTest` (permanent, Testcontainers-backed) as a regression test; updated `AuthServiceTest`'s mocks from `save` to `saveAndFlush`. Full suite: 19/19 pass.
